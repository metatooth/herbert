  ...
  query("SELECT * FROM clients WHERE client = $1", [client])
        .then(res => {
            if (res.rowCount === 0) {
                ...
                if (main) {
                    query("UPDATE clients SET main = $1, updated_at = CURRENT_TIMESTAMP WHERE client = $2", [main, client]);
                }
                
                if (intake) {
                    query("UPDATE clients SET intake = $1, updated_at = CURRENT_TIMESTAMP WHERE client = $2", [intake, client]);
                }

                if (profile) {
                    query("UPDATE clients SET profile = $1, updated_at = CURRENT_TIMESTAMP WHERE client = $2", [profile, client]);
                }
            }
        });
}
